...
const transactionId = `2022${getRandom(0, 1000)}07`

function AddNewSubject({ value = '', onClick = () => { } }) {
  return (
...
function CreateLoanModal({
  ...
  const [extraDocs, setExtraDocs] = useState(
    data
      ? data.otherDocs.map((v, i) => ({ key: i, label: v }))
      : [{ key: 0, label: '', isInput: true }]
  )
            ...
            <>
...

              {
                ...
                <>
                    ...
                    extraDocs.map((ed, i) => (
                      ...
                      >
                        <input
                          ...
                          value={ed.label}
                          onChange={e => setExtraDocs(prev => prev.map(p => {
                            if (p.key === i) {
                              return {
                                ...p,
                  ...
                  <Add
                    onClick={() => setExtraDocs(p => [...p, { key: p.length, label: '', isInput: true }])}
